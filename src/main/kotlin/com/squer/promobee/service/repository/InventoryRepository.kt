    ...
    fun getMonthlyAllocation(planId: String, userId: String,year: Int,month: Int): MutableList<AllocationInventoryDetailsWithCostCenterDTO> {
        ...
        if(user.userDesignation!!.id == UserRoleEnum.REGIONAL_BUSINESS_MANAGER_ID.id){
            ...
            bmDetails.forEach {
                ...
                if(bmPlan.isSpecial == 0 && bmPlan.planStatus!!.id == MonthlyPlanStatusEnum.APPROVED_ID.id){
...
//        data.put("isPopup", isPopup)
        return sqlSessionFactory.openSession().selectList<InventoryDTO?>("InventoryMapper.searchInventory", data)
    }
...
//        data.put("id", invId)
    ...
    fun reverseInventory(inv: List<ReverseInventoryDTO>) {
        ...
        inv.forEach {
            ...
            inv.get(i).invId?.let { it1 -> data.put("id", it1) }
            inv.get(i).quantity?.let { it1 -> data.put("quantity", it1) }
            inv.get(i).remarks?.let { it1 -> data.put("remarks", it1) }
...
//            inv.remarks?.let { data.put("remarks", it) }
            ...
            val inv1 = inv.get(i).invId?.let { getInventoryById(it) }
            ...
            if (inv1!!.qtyAllocated != null && inv1.qtyAllocated!! > 0 && inv.get(i).quantity!! > ((inv1.qtyReceived?.minus((inv1.qtyAllocated!! + inv1.qtyDispatched!!!!))).toString())) {

                inv.get(i).invId?.let { data.put("id", it) }

                sqlSessionFactory.openSession()
                    .selectList<InventoryReversalDTO>("InventoryMapper.getInventoryAllocatedByBM", data)
...
//        var inv2 =  InventoryService.getInventoryById(inv.invId)
            ...
            var inqd = qtyDisp?.plus(inv.get(i).quantity?.toInt()!!)
...
//        var inqd = inv1.qtyDispatched
            ...
            inv.get(i).invId?.let { data0.put("id", it) }
//        inv.quantity?.let { data0.put("qtyDispatched", it) }
            ...
            var reversalRemarks = it.remarks
            ...
            if(plan.size > 0){
                ...
                var idp = mutableListOf<InvoiceDetailPlan>();

                var data3: MutableMap<String, Any> = mutableMapOf()

                data3.put("planId",plan[0].id)

                idp = sqlSessionFactory.openSession().selectList<InvoiceDetailPlan>("InvoiceDetailPlanMapper.getInvoiceHeaderIdByPlan",data3)
                ...
                data4.put("id", indId1)
                data4.put("headerId", idp[0].headerId!!.id)
...
//                invoiceHeader.zip?.let { data3.put("zip", it) }
                ...
                if(it.remarks == "PRUNED"){
                    data3.put("designationId", ReversalRemarkEnum.PRUNED.id)
                }else if (it.remarks == "EXPIRED"){
                    data3.put("designationId", ReversalRemarkEnum.EXPIRED.id)
                }else if (it.remarks == "SHORT RECEIPT"){
                    data3.put("designationId", ReversalRemarkEnum.SHORT_RECEIPT.id)
                }else{
                    data3.put("designationId", ReversalRemarkEnum.DAMAGED.id)
                }
