...
//                saveAlloc[i].teamId?.let { it1 -> data1.put("teamId", it1) }
                    data.put("designationId",it.designationId!!)
                    data.put("teamId",it.teamId!!)
                    ...
                    recipient.forEach { it ->
                       ...
                       // var data: kotlin.collections.MutableMap<kotlin.String, kotlin.Any> = kotlin.collections.mutableMapOf()

                       // var data4: MutableMap<String, Any> = mutableMapOf()
                        // saveAlloc[i].inventoryId?.let { it1 -> data4.put("inventoryId", it1) }
                        data.put("inventoryId",invId!!)

                        var item = sqlSessionFactory.openSession().selectOne<ItemDrodownDTO>("ItemMapper.multipleAllocationByInventoryId",data)

                    //   var data5: MutableMap<String, Any> = mutableMapOf()

                        data.put("id",item.itemId!!)

                        var inventory = sqlSessionFactory.openSession().selectList<Inventory>("InventoryMapper.multipleAllocation",data)
...
//                                    kotlin.collections.mutableMapOf()
                                ...
                                data.put("id", inv.id)
                                data.put("qtyAllocated", invAllocatedQty!!)
                                data.put("updatedBy", user.id)

                                sqlSessionFactory.openSession().update("InventoryMapper.saveCommonAllocation", data)
        ...
        if(user.userDesignation!!.id == UserRoleEnum.REGIONAL_BUSINESS_MANAGER_ID.id){
            ...
            saveAlloc.forEach { it ->
                ...
                if(it.quantity!! > 0 ) {
                    ...
                    data3.put("inventoryId",invId!!)
                    data3.put("recipientId",employee.userRecipientId!!)
                    data3.put("qtyDispatch", dispatchedQuantity!!)

                    sqlSessionFactory.openSession().update("DispatchDetailMapper.updateCommonAllocation",data3)
...
//                    sqlSessionFactory.openSession().update("InventoryMapper.saveCommonAllocation", data3)
    ...
    fun createSpecialPlan(alloc: CreateAllocationDTO): CreateSpecialAllocationDTO {
        ...
        if (plan.isNullOrEmpty()) {
            ...
            var bmDetails = mutableListOf<DispatchDetail>()
            var employee = Users()
            var i = 0
            var bmPlan = DispatchPlan()
            var rbmDetails = mutableListOf<DispatchDetail>()
            ...
            if(user.userDesignation!!.id == UserRoleEnum.REGIONAL_BUSINESS_MANAGER_ID.id) {
                ...
                data0.put("year", alloc.year!!)
                data0.put("month", alloc.month!!)

                bmDetails = sqlSessionFactory.openSession().selectList<DispatchDetail>("DispatchDetailMapper.rbmStockSpecial",data0)

                bmDetails.forEach {
                    var data : MutableMap<String, String> = mutableMapOf()
                    data.put("planId", it.planId!!.id)

                    bmPlan = sqlSessionFactory.openSession().selectOne<DispatchPlan>("DispatchPlanMapper.monthlyAllocationDispatchPlanData",data)

                    data.put("planId", plan.id)
                    data.put("inventoryId",it.inventoryId!!.id)

                    rbmDetails = sqlSessionFactory.openSession().selectList<DispatchDetail>("DispatchDetailMapper.rbmDetailsMonthly",data)



                    if(bmPlan.isSpecial == 1 && bmPlan.planStatus!!.id == MonthlyPlanStatusEnum.APPROVED_ID.id){
                        ...
                        if(rbmDetails.isNullOrEmpty()){
                            ...
                            sqlSessionFactory.openSession().update("DispatchDetailMapper.rbmAllocationMonthlyUpdate",data)
                ...
                if(plan.planStatus!!.id == AllocationStatusEnum.SUBMIT.id || plan.planStatus!!.id == AllocationStatusEnum.APPROVED.id){
                    var data : MutableMap<String, String> = mutableMapOf()
                    data.put("RBMPlanID", plan.id)
                    data.put("UserID", employee.userRecipientId!!)
                    allocationInventoryDetails = sqlSessionFactory.openSession().selectList<AllocationInventoryDetailsWithCostCenterDTO>("InventoryMapper.rbmAllocationInventoryMonthly", data)
    ...
    fun searchSpecialPlan(month: Int, year: Int, status: String, remark: String): MutableList<DispatchPlan> {
        ...
        if (remark== "undefined" || remark.isNullOrEmpty()) {
            ...
            planList = plan.sortedByDescending { it.createdAt!! }.toMutableList()
    ...
    fun getRecipientForSpecialAllocation(ccmId: String): List<RecipientReportDTO> {
        ...
        return sqlSessionFactory.openSession()
            .selectList<RecipientReportDTO>("RecipientMapper.getRecipientSpecialAllocation", data)
...
//                    sqlSessionFactory.openSession().update("InventoryMapper.saveCommonAllocation", data3)
            ...
            saveAlloc.forEach { it ->
                ...
                if(it.quantity!! > 0) {
                    ...
                    if(dispatchedQuantity!! > inventoryStock ){
                        errorMap["message"] = "Allocation quantity is greater than the available stock for ${availableStock.poNo}"
                        errorMap["error"] = "true"
                        errorMap["info"] = "error"
                      ...
                      //  var data2: MutableMap<String, Any> = mutableMapOf()
                        ...
                        data.put("id", inventory[i].id)

                        var inv = sqlSessionFactory.openSession()
                            .selectOne<Inventory>("InventoryMapper.getInventoryByIdForInvoicing", data)
    ...
    fun deleteSpecialAllocation(dipId: String): Map<String, Any> {
        ...
        try {
            ...
            if (plan == null) {
                ...
                data.put("ID_DIP", dipId)

                sqlSessionFactory.openSession().update("DispatchPlanMapper.resetDraftPlan", data)

                data.put("id", dipId)

                sqlSessionFactory.openSession().delete("DispatchPlanMapper.deleteSpecialAllocation", data)
            ...
            saveAlloc.forEach {
                ...
                if(it.quantity!! > 0){
                 ...
                 //   var data2: kotlin.collections.MutableMap<kotlin.String, kotlin.Any> = kotlin.collections.mutableMapOf()
                    ...
                    data.put("designationId", designation!!)
...
//                    var inventoryStock = availableStock.qtyReceived!! -availableStock.qtyAllocated!!-availableStock.qtyDispatched!!
                    ...
                    data.put("planId",planId!!)
                    data.put("invId",invId!!)

                    virtualAllocatedStock = sqlSessionFactory.openSession().selectOne("DispatchDetailMapper.virtualAllocatedStockRBM",data)
                    ...
                    if(allocationQtySum > virtualAllocatedStock ){
                        ...
                        recipient.forEach { it ->
                          ...
                          //  var data: MutableMap<String, Any> = mutableMapOf()
                            ...
                            data.put("inventoryId", invId!!)
                            data.put("recipientId", it.id)
                            data.put("qtyDispatch", dispatchedQuantity!!.toString())
                            data.put("quarterlyPlanId", "00000000-0000-0000-0000-000000000000")
                            data.put("detailStatus", DispatchDetailStatusEnum.ALLOCATED.id)
                            data.put("createdBy", user.id)
                            data.put("updatedBy", user.id)
